...
export default function CapturePage() {
  ...
  async function startCamera() {
    ...
    try {
      ...
      // Força o play para evitar travas de renderização em navegadores mobile
      if (videoRef.current) {
  ...
  // Mudado para lidar com o vídeo rodando de verdade (essencial para Android)
  function handleVideoReady() {
    if (!autoCaptureDone) {
      setAutoCaptureDone(true);
      // Dá um pequeno tempo de segurança de 900ms com o vídeo já rodando antes de bater a foto
  ...
  function capturePhoto() {
    ...
    // Se o Android responder com dimensões zeradas no início, espera um pouco e tenta de novo
    ...
    canvas.toBlob(
      (blob) => {
        if (blob) {
          ...
          setMessage('Erro ao capturar a imagem. Tente novamente.');
        }
      },
      'image/jpeg',
      0.92
